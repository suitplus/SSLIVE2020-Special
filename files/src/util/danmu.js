...
function create_socket() {
    ...
    websocket.onclose = function (evt) {
        ...
        if (evt.code != 1006) {
            growl.show({ text: "正在重连", type: "custom", imgsrc: "/files/src/img/growl/loading.gif" });
            create_socket();
        }
    };
}
...
function send_danmu(text, color, size, position) {
    ...
    growl.show({ text: "正在发送", type: "custom", imgsrc: "/files/src/img/growl/loading.gif" });
}

function send_danmu_response(data) {
    if (data == "success") {
        growl.show({ text: "弹幕发送成功！", type: "custom", imgsrc: "/files/src/img/growl/ok.gif", autoclose: 1000 });
    } else if (data == 'block') {
